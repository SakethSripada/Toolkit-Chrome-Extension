document.addEventListener('DOMContentLoaded', function() {
  ...
  const linkInput = document.getElementById("add-link"); // Updated
  ...
  addRowButton.addEventListener("click", function() {    
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      if (!restrictedURLs.some(pattern => new RegExp(pattern).test(url))) {
        linkInput.value = url; // Updated
        const event = new Event('input', { bubbles: true });
        linkInput.dispatchEvent(event); // Updated
      }
    });
  });
  ...
  // Load stored rows from local storage and populate the table
  ...
  // Load stored rows when the page is loaded
  loadStoredRows();
});