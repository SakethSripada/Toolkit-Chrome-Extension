document.addEventListener('DOMContentLoaded', function() {
  ...
  let taskDataArray = JSON.parse(taskDataArrayJSON) || []; // Initialize if not present
...
window.onclick = function(event) {
  ...
  for (let hour = startHour ;hour <= endHour ;hour++) {
    ...
    timeSpan.addEventListener('click', () => {
      ...
      inputField.addEventListener('click', event => event.stopPropagation()) // Prevent propagation of click event to timeSpan
    ...
    // Listen for input change and save to local storage
