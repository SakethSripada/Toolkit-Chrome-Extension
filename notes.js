document.addEventListener('DOMContentLoaded', () => {
    ...
    let rightClickedIndex = -1 // Store the index of the right-clicked tab
    ...
    function renderNoteList() {
        ...
        const searchText = searchInput.value.toLowerCase() // Get search input text
        if (notes.length === 0) {
        ...
        notes.forEach((note, index) => {
            if (note.title.toLowerCase().includes(searchText) || note.body.toLowerCase().includes(searchText)) {
                ...
                // Show context menu (right-click menu) for deleting notes
    ...
    document.addEventListener('click', () => {
        contextMenu.style.display = 'none'
        rightClickedIndex = -1 // Reset the right-clicked index
    })
